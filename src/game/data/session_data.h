...
namespace game::data {
    ...
    public:
        ...
        void setNextLevel(const std::string& map_path);         // 设置下一个场景信息（地图、关卡开始时的得分生命）
        bool saveToFile(const std::string& filename) const;     // 将当前游戏数据保存到JSON文件（存档）
        bool loadFromFile(const std::string& filename);         // 从JSON文件中读取游戏数据（读档）
    };
