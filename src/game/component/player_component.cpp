...
namespace game::component {
	...
	void game::component::PlayerComponent::init()
	{
		...
		// 检查组件是否存在
		if (!transform_component_ || !physics_component_ || !sprite_component_ || !health_component_ || !animation_component_||!audio_component_) {
		...
		else { spdlog::error("初始化玩家状态失败（make_unique 返回空指针）！"); }
	...
	bool PlayerComponent::takeDamage(int damage)
	{
		...
		bool succsee = health_component_->takeDamage(damage);
		if (!succsee)return false;
