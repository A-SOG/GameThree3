...
namespace engine::render {
    ...
    private:
        ...
        //指向 SDL_Renderer 的非拥有指针

        engine::resource::ResourceManager* resource_manager_ = nullptr;
        //指向 ResourceManager 的非拥有指针


    public:
        //构造函shu
        //sdl_renderer 指向有效的 SDL_Renderer 的指针。不能为空。
        //resource_manager 指向有效的 ResourceManager 的指针。不能为空。
        ...
        */
        ...
        void present();//更新屏幕，包装 SDL_RenderPresent 函数

        void clearScreen();// 清屏，包装 SDL_RenderClear 函数

        void setDrawColor(Uint8 r, Uint8 g, Uint8 b, Uint8 a = 255);// 设置绘制颜色，包装 SDL_SetRenderDrawColor 函数，使用 Uint8 类型

        void setDrawColorFloat(float r, float g, float b, float a = 1.0f);//设置绘制颜色，包装 SDL_SetRenderDrawColorFloat 函数，使用 float 类型
