...
namespace engine::scene {
    bool LevelLoader::loadLevel(const std::string& level_path,
        Scene& scene) {
        ...
        if (!file.is_open())
        {
            spdlog::error("无法打开关卡文文件：{}", level_path);
            return false;
        }
    ...
    void LevelLoader::loadImageLayer(const nlohmann::json& layer_json, Scene& scene)
    {
        ...
        //获取视差因子
        const glm::vec2 scroll_factor = glm::vec2(layer_json.value
        ("parallaxx", 1.0f), layer_json.value("parallaxy", 1.0f));

        const glm::bvec2 repeat = glm::bvec2(layer_json.value
        ("repeatx", false), layer_json.value("repeaty", false));
    ...
    std::string LevelLoader::resolvePath(const std::string& relative_path, const std::string& file_path)
    {
        ...
        try {
            //获取地图文件的父目录“assets/maps/level1.tmj” -> “assets/maps”
