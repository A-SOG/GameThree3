...
namespace engine::component {
	...
	{// 如果没有正在播放的动画，或者没有当前动画，
		//或者没有精灵组件，或者当前动画没有帧，则直接返回
		...
		animation_timer_ += delta_time;// 推进计时器

		const auto& current_frame = current_animation_->getFrame(animation_timer_);// 根据时间获取当前帧
